...
rcon [0x01,0x01,0x00,0x02,0x00,0x04,0x00,0x08,0x00,0x10,0x00,0x20,0x00,0x40]
...
def key_xor(key,sbox_out,roundNum):
    ...
    rcon = rcon[roundNum-1] << 24
    #Get rcon+1 from table and bitshift by 24
    rcon_p1 = rcon[roundNum] << 24
    
    
    #Recombine, the upper bits use a different rcon due to the nature of AES192
    ...
    out_w2 = sbox_out ^ key_bin_split[1] ^ key_bin_split[0] ^ rcon_p1
    out_w3 = sbox_out ^ key_bin_split[0] ^ rcon_p1
