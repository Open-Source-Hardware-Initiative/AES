...
#This file contains a python demo of my combined lookup concept
debug = 0
...
#File to write test vectors to
tvFile = open("tv.txt","w")

#Loop numbervecs time with random data
for i in range(numberVecs):
    #Randomly Generate Inputs
    state = random.randint(1000,2**128-1)
    roundkey = random.randint(1000,2**128-1)
    
    #Generate output using round data function
    out = rounddata(state,roundkey)
    
    #Convert em all to hex
    state_hex = hex(state)[2:].zfill(32)
    roundkey_hex = hex(roundkey)[2:].zfill(32)
    out_hex = hex(out)[2:].zfill(32)
    
    #Write outputs to a file
    tvFile.write("%s_%s_%s\n" % (state_hex,roundkey_hex,out_hex))
