...
for i in range(vecNum):
    ...
    #Generate Input
    inData = random.randint(1000,2**128)
    inKey = random.randint(1000,2**128)
    
    #Generate Output
    encData = rounddata(inData,inKey)
    
    #Convert to hex
    hex_inData = hex(inData)[2:].zfill(32)
    hex_inKey = hex(inKey)[2:].zfill(32)
    hex_encData = hex(encData)[2:].zfill(32)
    
    #Write to test vector file
    tvFile.write("%s_%s_%s\n" % (hex_encData,hex_inData,hex_inKey))
